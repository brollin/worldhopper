...
void main() {
    ...
    vec3 color, black = vec3(0.);

    // Use polar coordinates instead of cartesian
    vec2 toCenter = vec2(.5) - st;
    float angle = atan(toCenter.y, toCenter.x);
    float normalizedAngle = (angle / TWO_PI) + .5;
    float radius = length(toCenter) * 2.;

    // Map the angle (-PI to PI) to the Hue (from 0 to 1)
    // and the Saturation to the radius
    color = hsb2rgb(vec3(sineIn(normalizedAngle), radius, 1.));

    gl_FragColor = vec4(distance(st, vec2(0.5)) > 0.5 ? black : color, 1.);
}
